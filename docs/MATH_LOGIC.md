...
**Version:** 5.0.33 (Updated April 2026)
**New in v5.0.33:** Word-count compliance — main body compressed from ~22,280 words to **~10,089** (Cardiff MSc limit 10,000).  No content lost; long sections moved to appendices rather than deleted.  Changes: (1) new Appendix E "Advanced Component Details" containing the 18 advanced System Design subsections (IRL, DFL, Risk-Adaptive Conformal, TFT, Causal RCT, Feature Store, Micro-Batch, Digital Twin, Drift Attribution, DRO-Fairness, Lipschitz, Safety, Counterfactual, Auto-scaling, Override, Rejections, SACT v4.1 Adapter, Stochastic MPC) with a forward-pointer paragraph in Chapter 4; (2) new Appendix F "Extended Evaluation" containing the 5 benchmark subsections (Weight-Sensitivity, External-Algorithm incl. Patrick 2008, Ablation, Component Significance, Fairness-SHAP) with per-subsection pointers in Chapter 5; (3) Literature Review compressed 1,969 → 862 words; (4) Methodology compressed 1,826 → 1,416 words; (5) all tables, figures, and live macros preserved unchanged. Final per-chapter word counts (main body only): Intro 974, Lit Review 862, Methodology 1,416, System Design 2,046, Results 2,787, Discussion 1,700, Conclusion 304 — total 10,089.  Cross-ref + bibliography smoke tests continue to pass.  PDF: 134 pages (appendices E+F carry the deferred content), 0 LaTeX errors, every `\ref` and `\citep` resolves.  Title-page word-count declaration updated from ≈20,900 → ≈10,089.
**New in v5.0.32:** Four integrated dissertation polish additions from the external review (no code change).  (1) New §4.1 TikZ block-diagram Figure "End-to-end architecture": three data channels → Feature Store → ML stack (Ensemble+DFL, TFT+GNN) → CP-SAT → Schedule, with dashed feedback loop (overrides via IRL → drift detection + recalibration → ML stack).  `tikz` + `tikz-arrows.meta/positioning/shapes.geometric` added to preamble.  (2) New §Prospective Validation Design: stepped-wedge cluster RCT protocol at Velindre (scheduling week = cluster unit, 24-week window, 96+96 cluster-days), primary outcome = 62-day NHS cancer-waiting compliance, secondaries = utilisation / no-show / NASA-TLX / F&FT, ITT mixed-effects logistic analysis powered to 5-pp compliance delta.  (3) New §Clinician Acceptance Protocol: 3-5 oncology nurses, 10 blinded scenarios each, forced-choice preference + thematic analysis with inductive double-coding (κ).  (4) New §Reflections on Advanced Methods: one honest sentence per component (IRL, DFL, TFT, GNN, column generation, conformal, DRO-fairness, slack-post-spread) naming the operating regime where each earns its place.  Cross-ref smoke test continues to pass: every \ref and \citep resolves; bibliography alphabetical-order test continues to pass.  PDF: 135 pages (was 131; +4 for architecture figure + three prospective-validation subsections), 0 LaTeX errors.
**New in v5.0.31:** Added Patrick, Puterman & Queyranne (2008) published operations-research baseline to §5.10 external-algorithm comparison (external-review: "compare against a published SACT scheduling algorithm").  New `_run_patrick_2008` function in `ml/benchmark_external_algorithms.py` implements the priority-first-fit myopic policy from their Operations Research 56(6) paper, adapted to the offline batch-solve setting: sort patients by priority class (P1 first) with earliest-time tie-breaking, greedy earliest-available chair within the per-patient window.  `dissertation_analysis.R §21-pre-pre-a` reads the new `patrick_points` and `hv_patrick` fields and emits 6 new `\extPatrick*` macros.  Table 5.6 gains a fourth row for Patrick 2008; §5.10 prose rewritten with a new "Honest finding — CP-SAT beats the published literature baseline" paragraph that quantifies the CP-SAT lead over Patrick using live macros (CP-SAT HV = `\extHVCpsat`, Patrick HV = `\extHVPatrick`).  Two new tests in `TestExternalAlgorithmBenchmark`: (a) `test_benchmark_row_schema` extended with `patrick_points` + `patrick_2008` in hypervolume dict; (b) `test_patrick_2008_ordering_is_priority_then_earliest_time` locks the priority-class-then-earliest-time sort so a future refactor cannot silently turn the arm into a different heuristic.  Live headline numbers on the n=30 / chairs=6 real cohort: CP-SAT HV = 0.3534, NSGA-II HV = 0.2280, Risk-greedy HV = 0.1152, Patrick 2008 HV = 0.0922 — a 3.8× CP-SAT lead over the published-literature baseline.  The prose frames this as the dissertation's concrete "advance over state-of-the-art" claim for this problem class.
**New in v5.0.30:** Full bibliography alphabetisation to resolve the external reviewer's recurring "missing reference" complaints.  All six entries the reviewer named (Walonoski, Gonzales, Grieves, Taskesen a/b, Desaulniers) were physically present in `main.tex` but six of them — plus Barnhart, Elmachtoub, Lim, Dwork, Bertsekas, Powell, Wilder — were orphaned out of alphabetical order at the tail of the bibliography.  A visual scan of the expected section (e.g., "G" for Grieves) found nothing, so the reviewer reasonably concluded the citations were unresolved.  Seven out-of-order tail entries relocated to their alphabetical positions; one in-list swap (Barnhart now precedes Benjamini).  `tests/test_reproducibility.py::test_bibliography_is_alphabetically_sorted` (1 new test, 1068 → 1069 passed) walks every `\bibitem[label]{key}`, extracts the first-author surname, and asserts the sequence is case-insensitively sorted — the reviewer's complaint pattern cannot silently recur.  Plus a hard-coded sanity check that the seven reviewer-named keys resolve.  PDF: 131 pages (was 132; page count dropped because the previous Table 5.3 caption expansion pushed a paragraph to a new page that alphabetisation pulled back), 0 LaTeX errors, every `\ref` and `\citep` resolves.
**New in v5.0.29:** Table 5.3 caption R(S) formula reminder — external-review point 7.  The reviewer noted Table 5.3 uses "Schedule robustness R(S)" without reminding the reader of the definition.  Extended the caption to inline the formula: "R(S) = mean(min(slack/60, 1)) over consecutive appointments on the same chair; the value is bounded in [0,1] with higher better (measured on the `\robustnessBenchN`-patient / `\robustnessBenchNchairs`-chair benchmark cohort from `ml/benchmark_robustness.py`)."  Uses live cohort macros so the caption self-documents both the formula and the experimental setup.  Prose-only fix: no benchmark change, no test change.  PDF: 132 pages (was 131; +1 because the caption grew past the page break), 0 LaTeX errors, every `\ref` resolves.
**New in v5.0.28:** Reconcile Table 5.6 (ablation) with Table 5.7 (bootstrap significance) — external-review point 6.  The reviewer flagged that the ablation table shows gender-ratio-to-zero on 4/5 ablated arms while the §5.12 bootstrap shows only column-generation crossing p<0.05 on utilisation.  A new paragraph "Single-run sensitivity vs. averaged effect — an honest caveat" added between the existing "Reading the table" paragraph and §5.12, explicitly naming the cohort sizes via live macros (`\abNpatients`/`\abNchairs` = 40/8 for ablation; `\csNpatients`/`\csNbootstrap` = 8/100 for bootstrap) and framing the two tables as answering different questions: Table 5.6 identifies components whose absence can push a SPECIFIC solve into a worst-case fairness outcome (a minority-gender cohort on a tightly-loaded grid); Table 5.7 answers whether the component's AVERAGE effect beats random cohort variation.  Both legitimate, both reported honestly.  Follow-up direction (larger-cohort bootstrap) named so the reviewer can see the next step.  Prose-only fix: no benchmark change, no test change.  PDF: 131 pages (page count held), 0 LaTeX errors.
**New in v5.0.27:** Two dissertation-prose polish items flagged in the external review.  (Point 4) §4.5 Treatment Duration Prediction now acknowledges the conformal under-coverage (empirical 85.7% vs target 90%) with a new sentence that frames it as within a Clopper–Pearson 95% binomial margin for n_test = 293 held-out appointments (±3.5 percentage points), and points to §Real-Data Validation Readiness for how a larger calibration split would tighten it without changing the finite-sample validity guarantee.  Uses live macros (`\cdEmpiricalCoverage`, `\cdTargetCoverage`, `\cdNtest`) so the margin text moves with the JSONL.  (Point 5) Figures 27 (Pareto util vs wait) and 28 (Pareto noshow vs robustness) captions now spell out the black-diamond's explicit coordinates using `\wsDefault*` live macros — e.g., "(wait=145.7 min, util=0.900)" for fig27 and "(noshow=0.169, R(S)=0.542)" for fig28 — so a reader does not have to squint at the axis marks to read the default point.  Both fixes are prose-only; no benchmark change.  PDF: 131 pages (page count held), 0 LaTeX errors, every `\ref` resolves.
**New in v5.0.26:** Fix §5.6.5 Fairness Mitigation cross-reference flagged in the external review (point 3).  The §5.6.5 prose said "the CP-SAT objective already carries the DRO-style fairness penalties of §\ref{sec:cpsat-impl}" — but `sec:cpsat-impl` is §4.5.1 (CP-SAT Implementation), which does not describe fairness penalties.  The correct target is §4.5.12 (Distributionally Robust Fairness Constraints, label `sec:dro-fairness`).  Changed the reference and also updated the sibling sentence's "`_fairness_constraints_enabled`" private-attribute mention to the current public `set_components(fairness=…)` API introduced in v5.0.19.  New `test_every_ref_target_has_matching_label` in `tests/test_reproducibility.py` parses `main.tex` for every `\ref` / `\autoref` / `\eqref` / `\pageref` / `\cref` / `\Cref` target and asserts each has a matching `\label` — so a future edit that introduces another broken cross-reference fails loudly.  Specific anchor checks for `sec:dro-fairness` and `sec:fairness-mitigation` lock the §5.6.5 fix against silent reversion.  PDF: 131 pages (page count held), 0 LaTeX errors, every `\ref` resolves.
**New in v5.0.25:** Fix missing Taskesen et al. (2021) citation flagged in the external review (point 2).  The reviewer reported the reference was not in the bibliography; on inspection, the existing `\bibitem` for `taskesen2021statistical` was orphaned out of alphabetical order (placed after Grieves / between Wolpert and Bertsekas) so a visual scan of the T section missed it.  Two fixes: (1) a new `\bibitem{taskesen2021aistats}` entry for the AISTATS 2021 paper "A distributionally robust approach to fairness in classification" (the original DRO-fairness bound source), (2) both Taskesen entries relocated to their alphabetical position between Sutton and Topol, (3) the §4.5.12 `\citep` updated to cite both papers.  New `test_every_citep_key_has_matching_bibitem` in `tests/test_reproducibility.py` walks `main.tex`, collects every `\citep{key}` and every `\bibitem{key}`, and asserts the set of cited keys is a subset of defined keys — plus hard-coded sanity checks that the two Taskesen keys resolve.  This catches both the severe failure mode (a citation with NO matching bibitem, which renders as `[?]` in the PDF) and prevents the reviewer's specific Taskesen finding from recurring.  PDF: 131 pages (was 130; +1 for the new bibliography entry), 0 LaTeX errors.
**New in v5.0.24:** Fix §5.9 text-vs-table inconsistency flagged in the external review (point 1).  An earlier draft of the "Reading the right panel" paragraph claimed that the black-diamond R(S) in Figure 5.9B, the R(S) cell in Table 5.3, and the Frontier B default in Figure 5.11 were "the same measurement" — they are not: Table 5.3 is the 40-patient / 8-chair / 10s cohort from `benchmark_robustness.py` (R(S) = 0.883 at w=0.10), whereas Figure 5.9B + Figure 5.11B are the 20-patient / 6-chair / 4s cohort from `benchmark_robustness_weight_sweep.py` + `benchmark_weight_sensitivity.py` (R(S) = 0.542 at w=0.10).  R(S) is cohort-dependent (slack geometry scales with chairs-per-patient).  Three new macros exposed so the §5.9 prose cites both cohort sizes live: `\robustnessBenchNchairs`, `\robustnessBenchTlim`, `\rwsNpatients`, `\rwsNchairs`, `\rwsTimeLimit` — §5.9 now renders "on the 20-patient / 6-chair sweep, default yields 0.542; on the 40-patient / 8-chair benchmark, default yields 0.883" with every number pulled from JSONL.  New `test_weight_sweep_and_table53_record_cohort_sizes` (1 new test, 1065 → 1066 passed) locks that both JSONL sources record their cohort sizes AND that the two cohorts differ — a future edit that merges them silently would break this test and the false "same measurement" claim could not re-enter.
**New in v5.0.23:** Figure 5.9 right panel (external-review Improvement I).  New `ml/benchmark_robustness_weight_sweep.py` sweeps the `robustness` objective weight over `{0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30}` on the real `patients.xlsx` cohort with deterministic CP-SAT (num_search_workers=1, random_seed=42) and per-weight fresh patient copies (avoiding DRO's in-place `expected_duration` mutation).  For each weight: records R(S) and the four slack-category fractions (Critical/Tight/Adequate/Ample).  `dissertation_analysis.R §7` reads the JSONL and builds a **combined two-panel Figure 5.9** via `patchwork`: Panel A = existing baseline slack-histogram, Panel B = R(S) vs w_robust line chart with the production default w=0.10 marked as a black diamond.  Falls back to two separate PDFs if `patchwork` is unavailable.  Emits 13 `\rws*` macros — source, n_weights, default_weight, default_rs, rs_at_{zero,ten,twenty,thirty}, plateau_weight, rs_min/max, crit_at_zero, ample_at_twenty.  Updated Figure 5.9 caption and new "Reading the right panel" paragraph cite the live macros so the prose moves with the benchmark.  `TestRobustnessWeightSweep` (3 new tests) locks row schema, R(S)/category-fraction ranges, and weak-monotone-non-decreasing of R(S) over w_robust.  Read-only Flask endpoint `/api/metrics/robustness-weight-sweep/status` re-serves the JSONL.  Live numbers (n=20 / chairs=6 / time_limit=4s / seed=42): R(S) response 0.000 → 0.275 → 0.542 → 0.817 → 1.000 → 1.000 → 1.000 at w ∈ {0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30}.  Plateau begins at w=0.20 (100% Ample transitions); production default w=0.10 yields R(S)=0.542 which matches §5.11 Frontier B default point.
**New in v5.0.22:** §3.7 Reproducibility Package (external-review Improvement H).  Multi-stage `Dockerfile` extended with a `reproducibility` target that runs the full pytest suite on a clean checkout: `docker build --target reproducibility -t sact-scheduler:repro .` then `docker run --rm sact-scheduler:repro`.  New `environment.yml` as a conda/mamba equivalent (73 lines, pins match the Docker python:3.12-slim base).  New `.zenodo.json` metadata file so tagging a release mints a versioned DOI via Zenodo's GitHub integration with zero manual work.  New `reproducibility/generate_manifest.py` captures git SHA + branch + dirty flag + Python/platform + sha256 checksums of every JSONL under `data_cache/` + `pip freeze` snapshot + Dockerfile/env.yml checksums into `reproducibility/manifest.json`; stdlib-only so it runs inside any Python 3.9+ environment (including a minimal Docker image).  `dissertation_analysis.R §22` reads the manifest and emits 12 `\repro*` macros; §3.7 "Container-based reproducibility" paragraph cites `\reproBuildCmd`, `\reproDockerCmd`, `\reproGitSha`, `\reproPython`, `\reproNjsonl`, `\reproNdeps`.  Read-only Flask endpoint `/api/reproducibility/status` re-serves the manifest for compliance review — no UI panel, never triggers a regeneration.  `TestReproducibilityArtefacts` (4 new tests): Dockerfile has reproducibility stage + `PYTHONHASHSEED=42` + pytest CMD; environment.yml parses as YAML with expected deps; .zenodo.json carries Zenodo-required fields; manifest generator produces a JSON file the R §22 reader walks.  Stale "961 tests" claim in §Reproducibility Statement replaced with a cadence description (actual count now lives in CI).
**New in v5.0.21:** §5.13 Model-Agnostic Root-Cause Analysis of Fairness Failures (external-review Improvement G).  New `ml/fairness_shap_explainer.py` trains a `GradientBoostingClassifier` base learner on `historical_appointments.xlsx` and computes `shap.TreeExplainer` attributions over the `patients.xlsx` cohort.  For every protected-attribute group-pair with ≥ 5 members each, it computes the mean per-feature SHAP gap and classifies the top-k contributors as `protected / proxy / geographic / clinical / temporal / administrative / other`.  A verdict per comparison: `bias_direct` (any top-k is `protected`), `legitimate_with_note` (any top-k is `proxy` or `administrative`), or `legitimate` (otherwise).  `dissertation_analysis.R §20e` reads the JSONL and emits 37 `\fsh*` macros covering provenance, Gender F-vs-M and Site largest-gap headline comparisons, and the top-3 feature contributors with category + gap.  New §5.13 subsection with Table 5.8 and honest verdict prose.  `TestFairnessShapExplainer` (3 new tests) locks row schema / sort invariant, feature-classifier buckets, and verdict-rule truth table.  Read-only Flask endpoint `/api/metrics/fairness-shap/status` returns the latest JSONL row for compliance review — no UI panel, never triggers a run.  Live finding on patients.xlsx head(120) + historical head(1500): Gender F-vs-M prediction gap = +0.025 explained by `expected_duration_min` (clinical), `travel_distance_km` (geographic), `has_comorbidities` (clinical); verdict = `legitimate`.  Site largest-gap comparison = `legitimate_with_note` where `contact_pref_phone` (administrative) appears in the top-3.
**New in v5.0.20:** §5.12 Statistical Significance of Component Effects (external-review Improvement F).  New `ml/benchmark_component_significance.py` runs a paired bootstrap for each of the five advanced CP-SAT components (column generation, GNN pre-filter, CVaR, fairness constraints, slack post-spread robustness).  Each replica draws a size-`n_patients` cohort with replacement from `patients.xlsx`, solves it twice (baseline / component-off via the public `set_components()` API), and records paired deltas on three metrics: utilisation, worst-pair Four-Fifths gender ratio, and solve time.  For each (component, metric) pair the harness reports mean Δ, 95% percentile bootstrap CI, and a two-sided bootstrap p-value `p = 2·min(P(Δ≤0), P(Δ≥0))` floor-clipped at `1/(B+1)`.  `dissertation_analysis.R §20d` reads the JSONL and emits 30 macros (5×3×2 for mean_delta + p_value) plus 7 provenance macros.  New §5.12 subsection with Table 5.7 (5 rows × 6 cols) + honest "reading the table" prose flagging that CG is `_cg_threshold=1`-forced, GNN has no training signal per-replica, and CVaR/fairness/robustness show within-noise effects at n=8 / B=100.  `TestComponentSignificanceBenchmark` (3 new tests) locks row schema, p-value+CI range invariants, and the `_bootstrap_stats` helper's agreement with a deterministic reference.  New Flask endpoint `/api/metrics/component-significance/status` returns the latest JSONL row as JSON (read-only, no UI panel, never triggers a run).
**New in v5.0.19:** Public `ScheduleOptimizer.set_components()` replaces direct mutation of the private `_cg_enabled` / `_gnn_enabled` / `_use_cvar_objective` / `_fairness_constraints_enabled` attributes.  Signature: `set_components(*, column_generation=None, gnn=None, cvar=None, fairness=None) -> Dict[str, bool]`.  Each argument defaults to `None` (leave current state alone); returned dict reflects post-update state for logging + assertions.  Two `__init__` additions: `_use_cvar_objective = True` and `_fairness_constraints_enabled = True` are now set explicitly so the method operates on a known baseline rather than whatever `getattr(…, True)` happened to return.  All seven external mutation call-sites migrated: `ml/benchmark_weight_sensitivity.py`, `ml/benchmark_ablation.py`, `ml/benchmark_fairness_mitigation.py`, `ml/benchmark_robustness.py`, `ml/benchmark_external_algorithms.py`, `ml/benchmark_column_generation.py`, plus the mutation paths in `tests/test_optimization.py` and `tests/test_column_generation.py`.  Flask read-only accesses in `flask_app.py` left as-is (legitimate diagnostic reads, not mutations).  New `TestSetComponents` (5 tests) locks the no-op, single-flag, all-flag, `__init__`-default, and bool-coercion invariants.  Pareto-front measurements unchanged after migration: default `(util=0.900, wait=145.7m, noshow=0.169, R(S)=0.542)`, Frontier A `default_dist=0.193`, Frontier B `default_dist=1.003`.
**New in v5.0.18:** §5.11 follow-through — reviewer cleanup pass on the weight-sensitivity harness.  (1) `mean_noshow_rate` field renamed to `mean_scheduled_noshow_rate` throughout `ml/benchmark_weight_sensitivity.py`, `dissertation_analysis.R §20a`, and `TestWeightSensitivityBenchmark` to make explicit that the metric averages no-show probability over *scheduled* patients only.  (2) Dead `--seed` CLI flag removed (nothing in the sweep path consumes it).  (3) Unused `horizon_min` parameter removed from `_score_point` and `_sweep_frontier` signatures (wait time is already bounded by chair closing time by construction).  (4) Stale comment that promised a second "fully-featured solver" default point deleted — overlaying a full-solver point on a reduced-solver frontier would mix trade-off surfaces and mislead readers.  (5) New `test_benchmark_runs_end_to_end` (opt-in via `RUN_SLOW_BENCHMARKS=1`) actually invokes `benchmark(n_patients=4, n_chairs=2, points_per_frontier=3, time_limit_s=2.0)` and validates the live JSONL row against all three schema-level invariants — the previous three tests locked fixtures only.  Pareto-front measurements and §5.11 figures/macros are byte-identical after the refactor: default `(util=0.900, wait=145.7m, noshow=0.169, R(S)=0.542)`, Frontier A `default_dist=0.193`, Frontier B `default_dist=1.003`.
**New in v5.0.17:** §5.11 Weight-Sensitivity Analysis (external-review Improvement E).  New `ml/benchmark_weight_sensitivity.py` sweeps two Pareto frontiers — utilisation-vs-waiting and noshow_risk-vs-robustness — by interpolating between each pair of weights while keeping the other four frozen at `OPTIMIZATION_WEIGHTS` defaults.  `dissertation_analysis.R §20a` reads the JSONL, generates two Pareto-front figures (`fig27_pareto_util_wait.pdf`, `fig28_pareto_noshow_robust.pdf`) with the production-default weights overlaid as a black-diamond marker, and emits 14 macros including the default's normalised distance to each frontier.  New §5.11 subsection with both figures + near-optimality prose citing the distance macros.  `TestWeightSensitivityBenchmark` (3 new tests) locks row schema, per-metric legal ranges, and monotonicity of weight interpolation.  Headline result on n=20 / chairs=6: Frontier A default_dist=0.193 (default near the front — near-optimal claim holds); Frontier B default_dist=1.003 (default off the front — §5.11 prose names the half-intensity post-spread mechanism rather than hiding it).
**New in v5.0.16:** §5.6.2 Improvement D — hard equalised-odds CP-SAT constraint.  New `ScheduleOptimizer._fairness_mode ∈ {"none","soft","hard"}` with `_fairness_hard_ratio = 0.85` default.  In "hard" mode the solver adds two-sided cross-multiplied constraints ($r_{g_1} \ge 0.85 \cdot r_{g_2}$ and vice versa) for every Age_Band + Gender pair with $\geq 3$ members, enforcing the Four-Fifths Rule directly.  The benchmark harness (`ml/benchmark_fairness_mitigation.py`) gains a third arm and excludes SACT-v4.0 "not stated" gender from the audit so missing data isn't confused with parity failure.  R `§21b` surfaces 7 new `\fm*Hard` macros; §5.6.2 prose rewritten with the three-column Off / Soft / Hard table and "Improvement D" paragraph.  `TestHardEqualisedOddsConstraint` (3 new tests) locks target-meeting, not-regressing-vs-soft, and configurable-threshold invariants.  Headline result on patients.xlsx head(60): Gender ratio SOFT = 0.595 → HARD = 0.943 (exceeds 0.85 target), utilisation cost 0.017 (inside <2% budget).
**New in v5.0.15:** §5.10 External-Algorithm Comparison (external-review Improvement C).  New `ml/benchmark_external_algorithms.py` implements three methods: (a) CP-SAT swept over the four production `PARETO_WEIGHT_SETS`, (b) a self-contained NSGA-II (~150 LOC, no pymoo dep) with non-dominated sorting + crowding distance + uniform crossover + random-reassignment mutation, (c) a risk-greedy heuristic sorted by no-show probability.  Scores all three on four normalised objectives (utilisation, P1 compliance, gender-fairness ratio, wait-score) and computes hypervolume (exact inclusion-exclusion for small Pareto sets).  `dissertation_analysis.R §21-pre-pre-a` reads the JSONL and emits 25 `\ext*` macros; new §5.10 subsection + Table 5.6 + honest "Reading the table" prose documenting the finding that NSGA-II has the larger hypervolume because the production scalarisation collapses gender fairness to zero on every preset.  `TestExternalAlgorithmBenchmark` (3 new tests): HV ∈ [0, 1], HV monotone under dominated-point addition, JSONL schema contract.  Headline numbers (n=30, chairs=6): HV(NSGA-II) = 0.228 > HV(risk-greedy) = 0.115 > HV(CP-SAT) = 0.000.
**New in v5.0.14:** §5.8 Ablation Study (external-review Improvement B).  New `ml/benchmark_ablation.py` runs the same cohort + chair grid seven times: baseline (all on) + five one-component-off arms (no-CG, no-GNN, no-fairness, no-CVaR, no-robustness).  Records utilisation, solve time, P1 compliance, and gender fairness ratio per arm.  `dissertation_analysis.R §21-pre-a` reads the JSONL and emits 28 macros covering all 6 arms × 4 metrics + the 4 provenance fields (source / n_patients / n_chairs / time_limit).  New §5.8 subsection + Table 5.5 + "Reading the table" prose.  `TestAblationSchema` (2 new tests) locks the 6-arm JSONL schema and the per-metric range invariants so a future harness refactor can't silently lose a Table 5.5 column.
**New in v5.0.13:** §4.7 Real-Data Validation Readiness — new `ml/real_data_channel.py` provides a four-gate detector (file presence + row count + SACT-v4.0 schema + anonymisation heuristic) returning `ChannelStatus(channel ∈ {synthetic, real}, reason, errors, warnings)`.  `ml/benchmark_real_vs_synthetic.py` runs a matched-seed no-show AUC head on both cohorts when real is present and records a dormant row (real_arm = null) when the detector says synthetic — so the dissertation pipeline is rehearsed every R run even while the DPIA is in flight.  `dissertation_analysis.R §20c` emits 11 `\rd*` macros; new §4.7 prose renders "channel = X / real_present = yes|no / synthetic AUC = Y / real AUC = n/a until dropped in".  New `/api/data/channel/real/status` endpoint returns the full detector verdict as JSON (read-only, no UI panel, no retrain).  `TestRealDataChannel` (6 tests) locks dormant-state, schema-rejection, anonymisation-warning, and end-to-end activation on a synthesised real-shaped file.  This is the external-review "Improvement A: Real-data validation" — infrastructure claim: when the Velindre cohort clears its DPIA, no code change is needed to produce the head-to-head table.
**New in v5.0.12:** §4.5 Treatment Duration Prediction — depth paragraph on conformal interval width vs MAE.  New `ml/benchmark_conformal_duration.py` runs a real head-to-head on the `historical_appointments.xlsx` cohort, computing MAE, interval-width, heteroscedasticity proxies (CV of widths, tail fraction > 2× median, Spearman(width, predicted)), and empirical coverage.  `dissertation_analysis.R §20b` reads the JSONL and emits 13 macros (`\cd{MAE,RMSE,WidthMean,WidthMedian,WidthPninety,CvWidth,TailFracTwoX,WidthVsPoint,EmpiricalCoverage,TargetCoverage,WidthOverMAE,Ntest,Source}`).  The dissertation prose replaces the stale 24.3/87 literals with live values and adds a two-bullet explanation citing heteroscedasticity + long-tail stats.  2 new regression tests in `TestConformalIntervalDepth` lock the width-over-MAE ratio ≥ 2 (heteroscedasticity check) and the empirical-coverage-within-tolerance invariant.
**New in v5.0.11:** §5.9 follow-through — implemented the slack-based robustness mechanism that was previously flagged as future work.  New `ScheduleOptimizer._redistribute_for_robustness` post-process spreads each chair's appointments evenly within their per-patient $[\text{earliest}, \text{latest}]$ windows when the `robustness` weight is non-zero; the CP-SAT solver still picks every chair assignment and patient ordering, only the within-chair time-axis layout is adjusted.  Re-running `ml/benchmark_robustness.py` on the same 40-patient cohort flips R(S) from 0.000 (weight 0.00) to **0.883** (weight 0.10) — Δ R(S) = +0.883, all 100 % Critical → 100 % Adequate transitions.  The dissertation §5.9 prose now drops the "future work" caveat and locks the behaviour with a new `test_slack_redistribution_increases_rs` regression (asserts $\Delta R(S) > 0.05$ on a fixed 12-patient / 4-chair cohort).
**New in v5.0.10:** §5.9 regression — `ml/benchmark_robustness.py` replaces the fabricated 0.098 baseline / 0.135 ± 0.028 optimised numbers in Table 5.3 with real R(S) measurements on the optimiser's own output.  The R formula `mean(min(slack_min/60, 1))` over consecutive chair transitions is mirrored verbatim in Python so dissertation_analysis.R §21a and the Python benchmark compute the same quantity.  New macros `\robustnessBaselineActual`, `\robustnessOptimiserWeightZero`, `\robustnessOptimiserWeightTen`, `\robustnessDelta`, `\robustnessBenchSource`, `\robustnessBenchN` are all sourced from the same JSONL row, so Figure 6 + Table 5.3 cannot drift.  Figure 6 caption corrected — it renders the single actual-schedule histogram, not a baseline-vs-optimised split (the earlier "Left: baseline; Right: system" caption was wrong).  Honest caveat documented: the CP-SAT `robustness` weight penalises long treatments, not slack directly — a slack-based objective is flagged as future work.  Two new `TestRobustnessMetric` tests lock the Python/R formula equivalence and the JSONL schema.
